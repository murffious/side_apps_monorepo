...
# - If src/components/api is not empty, inject API_ENFORCEMENT_PROMPT
# - If src/components/mcp is not empty, inject MCP_ENFORCEMENT_PROMPT
#
...
- **REQUIRED**: Use the 'raf-data-generator' subagent to analyze the data layer requirements and handle ORM operations.
