...
export default {
  'no-empty-select-item': {
    meta: {
      ...
      messages: {
        emptySelectValue:
          'SelectItem cannot have empty string value. Use a non-empty value like "none" instead.',
      },
    },
                ...
                context.report({
                  ...
                  fix: hasFix
                    ? function (fixer) {
                        ...
                        if (!isQuiet) {
                          console.error(
                            `  ✓ Fixed empty SelectItem value at ${filename}:${loc.start.line}:${loc.start.column}`
                          );
                        }
