...
function LoginPage() {
  ...
  const { login, signup, confirmSignup, resendCode, isAuthenticated } =
    useAuth();
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleVerification} className="space-y-4">
                ...
                {verificationError && (
                  <p
                    className={`text-sm ${verificationError.includes('successfully') ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}
                  >
                    {verificationError}
                  </p>
                )}
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mb-1">
            <span className="line-through opacity-50">Achieving</span> =
            Destination thinking. "I want to <em>have done</em> X"
          </p>
          <p className="text-sm font-medium text-zinc-700 dark:text-zinc-300">
            <span className="text-amber-600 dark:text-amber-500">Becoming</span>{' '}
            = Journey thinking. "I want to <em>be someone who does</em> X"
          </p>
        </div>{' '}
