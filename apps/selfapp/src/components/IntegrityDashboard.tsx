...
export function IntegrityDashboard() {
	...
	const handleSaveAlignment = () => {
		if (
			!currentAlignment.desire &&
			!currentAlignment.intention &&
			!currentAlignment.choice &&
	...
	const generateAISuggestions = (): string[] => {
		...
		if (alignmentLayers.length === 0) {
			suggestions.push(
				"Start tracking your alignment layers to receive personalized suggestions.",
			);
			return suggestions;
		}
		...
		// Check for patterns
		const hasDesireGap = recentAlignments.some(
			(a) => a.desire && (!a.intention || !a.action),
		);
		const hasIntentionGap = recentAlignments.some(
			(a) => a.intention && !a.action,
		);
		const hasChoiceGap = recentAlignments.some((a) => a.choice && !a.action);

		if (hasDesireGap) {
			suggestions.push(
				"Consider creating specific intentions and action plans for your desires to improve alignment.",
			);
		}

		if (hasIntentionGap) {
			suggestions.push(
				"You have intentions but missing actions. Break down your intentions into smaller, actionable steps.",
			);
		}

		if (hasChoiceGap) {
			suggestions.push(
				"Transform your choices into concrete actions. Set specific times and conditions for execution.",
			);
		}

		if (alignmentLayers.length >= 7) {
			const avgScore =
				alignmentLayers.reduce((sum, layer) => {
					let layerScore = 0;
			...
			if (avgScore < 75) {
				suggestions.push(
					"Your integrity score could improve. Focus on completing all four layers consistently.",
				);
			} else if (avgScore >= 90) {
				suggestions.push(
					"Excellent alignment! You're demonstrating strong integrity across all layers.",
				);
			}
		}
		...
		if (suggestions.length === 0) {
			suggestions.push(
				"Keep logging your alignment layers to build a strong foundation of integrity.",
			);
		}
		...
		if (settings.showBiblicalContent) {
			const interval = setInterval(() => {
				setCurrentExcerptIndex(
					(prev) => (prev + 1) % motivationalExcerpts.length,
				);
	...
	return (
		<div className="space-y-6">
			...
			{showSettings && (
				<Card>
					<CardHeader>
						...
						<CardDescription>
							Customize your integrity dashboard layout and preferences
						</CardDescription>
					</CardHeader>
								...
								{Object.entries(settings.visibleWidgets).map(([key, value]) => (
									<div key={key} className="flex items-center justify-between">
										...
										<Switch
											...
											onCheckedChange={() =>
												toggleWidget(
													key as keyof IntegritySettings["visibleWidgets"],
												)
											}
										/>
									</div>
			...
			{settings.showBiblicalContent &&
				settings.visibleWidgets.biblicalContent && (
					...
					<CardContent>
							...
							{[
								{
									...
									filled:
										...
										alignmentLayers[alignmentLayers.length - 1]?.desire,
								},
								{
									label: "Intention",
									icon: Target,
									filled:
										alignmentLayers.length > 0 &&
										alignmentLayers[alignmentLayers.length - 1]?.intention,
								},
								{
									label: "Choice",
									icon: TrendingUp,
									filled:
										alignmentLayers.length > 0 &&
										alignmentLayers[alignmentLayers.length - 1]?.choice,
								},
								{
									label: "Action",
									icon: Eye,
									filled:
										alignmentLayers.length > 0 &&
										alignmentLayers[alignmentLayers.length - 1]?.action,
								},
							].map((layer) => (
								<div
									key={layer.label}
								...
								>
									<layer.icon
										className={`h-5 w-5 mx-auto mb-2 ${layer.filled ? "text-green-600 dark:text-green-400" : "app-text-muted"}`}
									/>
						...
						<div className="mt-4 p-3 rounded-lg app-bg-surface-alt">
							<p className="text-xs app-text-subtle">
								<strong>How it's calculated:</strong> Each completed layer
								(Desire, Intention, Choice, Action) contributes 25 points to
				...
				<Card>
					...
					<CardContent className="space-y-4">
						<div className="space-y-4">
							<div className="space-y-2">
								<Label htmlFor="desire">
									1. Desire - What do you truly want?
								</Label>
								<Textarea
									id="desire"
							...
							<div className="space-y-2">
								<Label htmlFor="intention">
									2. Intention/Plan - What do you intend to do?
								</Label>
								<Textarea
									id="intention"
							...
							<div className="space-y-2">
								<Label htmlFor="choice">
									3. Choice - What choice are you making?
								</Label>
								<Textarea
									id="choice"
							...
							<div className="space-y-2">
								<Label htmlFor="action">
									4. Action - What action will you take?
								</Label>
								<Textarea
									id="action"
						...
						{alignmentLayers.length > 0 && (
							<div className="border-t pt-4 mt-4">
								<h4 className="font-semibold mb-3 app-text-strong">
									Recent Alignments
								</h4>
