...
export default {
  'mcp-test-first': {
    meta: {
      ...
      docs: {
        description:
          'Require MCP endpoint testing verification before integration',
      ...
      messages: {
        missingVerification:
          'MCP integration must include verification comment showing endpoint was tested. Add a comment like: "// VERIFIED: Tested with curl - correct parameter names for {{toolName}}"',
        missingToolName:
          'MCP integration verification comment should include the tool name being called',
        assumedParameters:
          'MCP integration uses common parameter names ({{params}}) that should be verified. These often differ from standard REST API patterns.',
      },
    },
    create(context) {
      ...
      function extractToolName(node) {
        // Look for callTool(serverUrl, "TOOL_NAME", args)
        if (
